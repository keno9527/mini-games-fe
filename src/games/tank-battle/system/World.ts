...
import { LEVELS, PLAYER_SPAWN_CELL } from '../data/levels.ts';
import { Base } from '../entity/Base.ts';
import type { Bullet } from '../entity/Bullet.ts';
import { Explosion } from '../entity/Explosion.ts';
import type { PowerUp } from '../entity/PowerUp.ts';
import { Tank } from '../entity/Tank.ts';
import { Direction, TankSide, type EnemyKind } from '../types.ts';
import { TerrainGrid } from './TerrainGrid.ts';
...
export class World {
  ...
  onBaseDestroyed(): void {
    ...
    this.explosions.push(
      new Explosion(rect.x + rect.width / 2, rect.y + rect.height / 2, true),
    );
    this.outcome = LevelOutcome.FAILED;
  }
  ...
  private addExplosion(tank: Tank | null, big: boolean): void {
    ...
    this.explosions.push(
      new Explosion(rect.x + rect.width / 2, rect.y + rect.height / 2, big),
    );
  }
