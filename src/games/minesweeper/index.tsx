...
export default function Minesweeper({ userId, gameId }: Props) {
  ...
  const submitRecord = useCallback(
    async (result: 'win' | 'lose', duration: number, score: number) => {
  ...
  return (
    <div className="flex flex-col items-center gap-5">
      ...
      <div className="overflow-auto max-w-full" style={{ userSelect: 'none' }}>
        {status === 'idle' && (
          <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${cfg.cols}, 28px)` }}>
          ...
          <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${cfg.cols}, 28px)` }}>
            {board.map((row, r) =>
              row.map((cell, c) => {
                let content = ''
                let cellCls =
                  'w-7 h-7 rounded-lg text-xs flex items-center justify-center select-none transition-colors '
                if (cell.revealed) {
                  if (cell.mine) {
                    ...
                    cellCls +=
                      'bg-blue-50 border-2 border-blue-100 ' + (numColors[cell.adjacent] ?? '')
